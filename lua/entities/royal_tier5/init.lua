...
function ENT:Initialize()
	...
	self:Setpmaxhold(pconfig.tier5.maxhold)
	self:Setpersecond(pconfig.tier5.persecond)
	self:Setmaxlevel(pconfig.tier5.maxlevel)
	self.maxincrease = pconfig.tier5.mincrease
	self.pincrease = pconfig.tier5.pincrease
...
function ENT:Use(activator, caller)
	...
	if localTrace and wait == false then
		if localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < -2 and localTrace.x > -6 then
			if activator:canAfford(pconfig.tier5.upgradecost * self:Getpmoneylevel()) then
				if self:Getpmoneylevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier5.upgradecost * self:Getpmoneylevel())
		...
		elseif localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < 10 and localTrace.x > 6 then
			if activator:canAfford(pconfig.tier5.upgradecost * self:Getpholdlevel()) then
				if self:Getpholdlevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier5.upgradecost * self:Getpholdlevel())

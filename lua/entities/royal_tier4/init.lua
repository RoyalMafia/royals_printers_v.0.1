...
function ENT:Initialize()
	...
	self:Setpmaxhold(pconfig.tier4.maxhold)
	self:Setpersecond(pconfig.tier4.persecond)
	self:Setmaxlevel(pconfig.tier4.maxlevel)
	self.maxincrease = pconfig.tier4.mincrease
	self.pincrease = pconfig.tier4.pincrease
...
function ENT:Use(activator, caller)
	...
	if localTrace and wait == false then
		if localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < -2 and localTrace.x > -6 then
			if activator:canAfford(pconfig.tier4.upgradecost * self:Getpmoneylevel()) then
				if self:Getpmoneylevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier4.upgradecost * self:Getpmoneylevel())
		...
		elseif localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < 10 and localTrace.x > 6 then
			if activator:canAfford(pconfig.tier4.upgradecost * self:Getpholdlevel()) then
				if self:Getpholdlevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier4.upgradecost * self:Getpholdlevel())

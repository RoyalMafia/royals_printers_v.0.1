...
function ENT:Initialize()
	...
	self:Setpmaxhold(pconfig.tier6.maxhold)
	self:Setpersecond(pconfig.tier6.persecond)
	self:Setmaxlevel(pconfig.tier6.maxlevel)
	self.maxincrease = pconfig.tier6.mincrease
	self.pincrease = pconfig.tier6.pincrease
...
function ENT:Use(activator, caller)
	...
	if localTrace and wait == false then
		if localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < -2 and localTrace.x > -6 then
			if activator:canAfford(pconfig.tier6.upgradecost * self:Getpmoneylevel()) then
				if self:Getpmoneylevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier6.upgradecost * self:Getpmoneylevel())
		...
		elseif localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < 10 and localTrace.x > 6 then
			if activator:canAfford(pconfig.tier6.upgradecost * self:Getpholdlevel()) then
				if self:Getpholdlevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier6.upgradecost * self:Getpholdlevel())

...
function ENT:Draw()
	...
	cam.Start3D2D(Pos + Ang:Up() * 10.8, Ang, 0.11)
		...
		draw.RoundedBox(0, -140, -150, 283, 40, Color(60,230,60))
		draw.RoundedBox(0, -140, -110, 283, 4, Color(50,140,50))
		draw.SimpleText("Tier III Printer", "royals_font_1", 0, -130, Color(30,30,30), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		if alpha > 0 then
			draw.RoundedBox(0, -120, -90, 243, 40, Color(60,230,60,alpha))
			draw.RoundedBox(0, -120, -50, 243, 30, Color(50,140,50,alpha))
			...
			if MoneyLvl != MaxLevel then
				draw.SimpleText("Upgrade ("..DarkRP.formatMoney(pconfig.tier3.upgradecost * self:Getpmoneylevel())..") "..MoneyLvl.."/"..MaxLevel - 1, "royals_font_3", 0, -37, Color(30,30,30,alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			else
				draw.SimpleText("Max Level", "royals_font_3", 0, -37, Color(30,30,30,alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end

			draw.RoundedBox(0, -120, 20, 243, 40, Color(60,230,60,alpha))
			draw.RoundedBox(0, -120, 60, 243, 30, Color(50,140,50,alpha))
			...
			if HoldLvl != MaxLevel then
				draw.SimpleText("Upgrade ("..DarkRP.formatMoney(pconfig.tier3.upgradecost * self:Getpholdlevel())..") "..HoldLvl.."/"..MaxLevel - 1, "royals_font_3", 0, 73, Color(30,30,30,alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			else
	...
	cam.Start3D2D(Pos + Ang:Up()*16.7, Ang, 0.11)
		draw.RoundedBox(0, -131, -63, 200, 30, Color(60,230,60))

		draw.RoundedBox(0, -131, -67, 200, 4, Color(50,140,50))
		draw.RoundedBox(0, -131, -33, 200, 4, Color(50,140,50))

		draw.SimpleText("Tier III Printer", "royals_font_1", -30, -49, Color(30,30,30), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end

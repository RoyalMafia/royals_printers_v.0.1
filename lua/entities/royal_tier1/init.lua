...
function ENT:Initialize()
	...
	self:Setpmaxhold(pconfig.tier1.maxhold)
	self:Setpersecond(pconfig.tier1.persecond)
	self:Setmaxlevel(pconfig.tier1.maxlevel)
	self.maxincrease = pconfig.tier1.mincrease
	self.pincrease = pconfig.tier1.pincrease
...
function ENT:Use(activator, caller)
	...
	if localTrace and wait == false then
		if localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < -2 and localTrace.x > -6 then
			if activator:canAfford(pconfig.tier1.upgradecost * self:Getpmoneylevel()) then
				if self:Getpmoneylevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier1.upgradecost * self:Getpmoneylevel())
				...
				elseif self:Getpmoneylevel() == self:Getmaxlevel() then
					net.Start("rm_notify")
						net.WriteString("Reached max upgrade level!")
					net.Send(activator)
				end
			else
				net.Start("rm_notify")
		...
		elseif localTrace.y > -13.5 and localTrace.y < 13.5 and localTrace.x < 10 and localTrace.x > 6 then
			if activator:canAfford(pconfig.tier1.upgradecost * self:Getpholdlevel()) then
				if self:Getpholdlevel() < self:Getmaxlevel() then
					activator:addMoney(-pconfig.tier1.upgradecost * self:Getpholdlevel())
